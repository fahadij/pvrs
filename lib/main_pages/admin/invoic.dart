...
class InvoicePageAdmin extends StatefulWidget {
  @override
  _InvoicePageAdminState createState() => _InvoicePageAdminState();
}

class _InvoicePageAdminState extends State<InvoicePageAdmin> {
  List<Map<String, dynamic>> invoices = [];


  Future<void> fetchInvoicesAdmin() async {
    String? token2;
    ...
    var results = await conn.execute(
        "SELECT * FROM invoice ORDER BY invoice_num ASC",

    );

var number5 = results.numOfRows;
    List<Map<String, dynamic>> list = [];
print ("this is the number of invoices$number5");
    for (final row in results.rows) {
      final data = {
        ...
        'invoice_renter': row.colByName("user_id"),
      };
      list.add(data);
    }
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          ListTile(
            title: Text('Renter ID: ${invoice['invoice_renter']}'),
          ),
          ListTile(
