...
}else if(!empty($_POST['email_admin'])){
    $mail3 = $_POST['email_admin'];
}
else{ $mail3 = ""; echo json_encode(["error" => "Message could not be sent. error on email:$mail3"]);}
...
elseif(!empty($_POST['email_admin'])){
    $sqlQuery1 =  "UPDATE admin SET otp = '$otp' WHERE admin_id = {$_POST['id']}";
    $conn->query($sqlQuery1);
}
